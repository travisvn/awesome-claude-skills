...
| **[deep-context](https://github.com/googlarz/deep-context)** | Builds verified context packs from folders (≤200 files): per-file notes with mandatory source citations, contradiction detection, independent red-team pass, self-test calibration, and a one-page actionable digest. For legal, financial, medical, and audit corpora where a confidently-wrong answer is expensive. CC BY-NC 4.0 |
| **[health-skill](https://github.com/googlarz/health-skill)** | Persistent health workspace and longevity companion — connects labs, medications, family history, symptoms, check-ins, and training across sessions. Drug interaction checker, pharmacogenomics (23andMe/AncestryDNA), FHIR R4 import, cycle tracking, menopause module, men's health module, preventive care, cross-domain pattern detection, nutrition tracker, Monte Carlo projections, caregiver dashboard for whole households. 237 tests. v2.2 |
| **[logic-audit](https://github.com/googlarz/logic-audit)** | Cross-artifact consistency auditor — finds bugs that live between files: spec says return None, code raises, tests pass against the wrong behavior. Works on code+tests+docs, contracts, financial reports, research data. 6-phase audit with structural verification gates, evidence grading with hard ceilings, and differential mode |
| **[math-skill](https://github.com/googlarz/math-skill)** | Rigorous math solver with SymPy verification, matplotlib visualization, and photo input (handwritten problems, whiteboards). Modes: solver, tutor, proof, exam, worked-solution, lesson. Verifies its own answers programmatically |
| **[finance-assistant](https://github.com/googlarz/finance-assistant)** | Personal finance copilot: 12 modes including budget manager, debt optimizer, investment tracker, tax module (DE/UK/FR/NL/PL — validated against 29 official test cases), Monte Carlo FIRE projections, scenario lab, bank import (CSV/MT940/OFX). Encrypted SQLite, privacy-first |
| **[betriebsrat](https://github.com/googlarz/betriebsrat)** | German labour law advisor grounded in gesetze-im-internet.de — BetrVG, KSchG, AGG, MuSchG. Reasons through procedure, deadlines, and entitlements for employees and works council members. Every answer cites the specific paragraph with a verification link |
| **[fashion-skill](https://github.com/googlarz/fashion-skill)** | Personal AI stylist: body analysis from photos, wardrobe inventory, outfit recommendations with reasoning, pre-purchase checks, in-store mode, week-ahead planning with weather, feedback loop that learns from what you actually wear, Zalando price alerts, Vinted resell listing generator |
| **[deterministic-workflow-builder](https://github.com/googlarz/deterministic-workflow-builder)** | Turns vague agent workflows into auditable packages: typed workflow.json manifest, steps/*.sh, approval gates, machine-checkable artifact contracts, parallel DAG execution, rollback hooks, doctor/repair commands, bounded AI sidecars. For production agent pipelines that cannot fail silently |
